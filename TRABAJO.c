...
int operaciones () {
    ...
    int posMin = minFromFile(filename, dim);
	...
	switch (opcion) {
		...
		case 2:
			do{
		    	...
		    	scanf("%d", &num);
		    	if (posMin >= 0) {
                    printf("El minimo valor de 'ph' se encuentra en la posicion %d.\n", posMin);
				} 
				else {
                printf("Error al calcular el minimo valor.\n");
			 }
		...
		case 3:
			do{
		    	...
		    	scanf("%d", &num);
		    	if (posMax >= 0) {
                    printf("El máximo valor de 'ph' se encuentra en la posicion %d.\n", posMax);
				} 
				else {
...
int minFromFile(const char* filename, int dim) {
    FILE* archivo = fopen(filename, "r");
    if (archivo == NULL) {
        printf("Error al abrir el archivo.\n");
        return -1;
    ...
    struct dato datoMenorExtension;
    int i, posMin;
    
    // Leer el primer dato del archivo
    fscanf(archivo, "%f %d %d %d", &datoMenorExtension.ph, &datoMenorExtension.conductividad, &datoMenorExtension.turbidez, &datoMenorExtension.coliformes);
    posMin = 0;
    ...
    for (i = 1; i < dim; i++) {
        ...
        if (datos.ph > datoMenorExtension.ph) {
            datoMenorExtension = datos;
            posMin = i;
        }
    }
